# These are the solved exercises from Lecture 2
# 9/5/19
...
ratings_factor <- ratings_5 %>%
  mutate_at(vars(MPRATING),funs(factor)) %>%
...
# Summary stats for the share of largest city in each country (in html--so can be viewed in a web browser)
stargazer(as.data.frame(total_urban), type = "html")
